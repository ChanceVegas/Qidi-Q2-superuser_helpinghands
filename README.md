...
Firmware note: Qidi Q2 firmware 1.1.2 / `V01.01.02.01` uses a new `/home/qidi` + `qidi-client` layout. AIO RC2.31 resolves that layout internally, allows option 8 read-only diagnostics and restore-contract verification, option 4 dry-run Revert reporting plus guarded baseline/restore-contract capture, options 9-13's staged compatibility/restore proofs, and options 14-15's independent controlled restore proofs for the loaded stock Klipper extras and Moonraker components directories. Full install/general real-revert/addon/repair paths remain blocked until the dedicated compatibility lane is implemented.
