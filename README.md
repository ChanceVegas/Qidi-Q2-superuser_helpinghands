...
Firmware note: Qidi Q2 firmware 1.1.2 / `V01.01.02.01` uses a new `/home/qidi` + `qidi-client` layout. AIO RC2.23 resolves that layout internally, allows option 8 read-only diagnostics, and allows option 4 dry-run Revert reporting, while install/real-revert/addon/repair paths remain blocked until the dedicated compatibility lane is implemented.
