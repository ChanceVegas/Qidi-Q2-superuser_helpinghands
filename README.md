...
Firmware note: Qidi Q2 firmware 1.1.2 / `V01.01.02.01` uses a new `/home/qidi` + `qidi-client` layout. AIO RC2.22 resolves that layout internally and allows option 8 read-only diagnostics, while install/revert/addon/repair paths remain blocked until the dedicated compatibility lane is implemented.
