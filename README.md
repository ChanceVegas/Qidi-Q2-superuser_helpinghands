...
Firmware note: Qidi Q2 firmware 1.1.2 uses a new `/home/qidi` + `qidi-client` layout. AIO RC2.36 resolves that layout internally, allows option 8 read-only diagnostics and restore-contract verification, option 4 dry-run Revert reporting plus guarded baseline/restore-contract capture, options 9-16's staged compatibility/restore proofs, and option 17's guarded schema-2 contract refresh after a verified Qidi stock-package upgrade. Option 17 accepts only package-verified config updates and known mutable Klipper state, and searches known Qidi/runtime/backup trees for provenance when active config files are unowned; all other config drift remains blocked. Full install/general real-revert/addon/repair paths remain blocked until the dedicated compatibility lane is implemented.
