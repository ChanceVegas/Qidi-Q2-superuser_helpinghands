...
```
============================================
   Qidi Q2 Superuser - AIO Setup Menu (RC2.23)
============================================
...
Qidi Q2 firmware 1.1.2 / `V01.01.02.01` migrates the printer to `/home/qidi`, leaves `/home/mks` as a symlink, replaces `makerbase-client` with `qidi-client.service`, and moves stock macros under `klipper-macros-qd/`. RC2.23 detects that layout, resolves the active home/config/service names internally, allows option 8 read-only diagnostics and option 4 dry-run revert reporting, and still blocks install, real revert, addon, and verifier repair paths until the dedicated 1.1.2 compatibility lane is implemented.
...
| 4 | **Revert to Backup** | On supported layouts, fully uninstalls everything AIO has installed and restores your printer to the state it was in before the first AIO run. On unsupported layouts such as Q2 firmware 1.1.2, runs a dry-run report only: no restore, deletion, or service changes. |
...
On unsupported layouts such as Q2 firmware 1.1.2, option 4 is dry-run only. It reports the selected backup source, what would be restored, what AIO artifacts would be removed, which stock files/services must be preserved, Qidi Box objects/sensors, and the active include graph. It does not run `rsync`, delete files, edit configs, or change services.
...
- **Qidi Q2 firmware 1.1.2 is detected but not yet supported for mutating actions.** AIO resolves the new `/home/qidi` + `qidi-client` layout, option 8 can run read-only diagnostics, and option 4 can run a dry-run Revert report. Install, real revert, addon, and repair paths remain blocked until the compatibility lane lands.
